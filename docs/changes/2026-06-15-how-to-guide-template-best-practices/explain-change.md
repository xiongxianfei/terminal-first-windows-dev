...
| `tests/markdown/how-to-guide-template-best-practices.test.sh` | Added and expanded the focused proof script for template structure, metadata, conditional modules, command-block conventions, guide conformance, evidence records, troubleshooting link/anchor resolution, backup cues, pilot path compatibility, and first-slice guardrails. | Provides tests/proof before implementation and gives reviewers a repeatable static check without executing setup commands. | Test spec T1-T8; plan M1-M3 validation direction. | Initial M1/M2 runs exposed expected missing surfaces; M3 final proof passed. |
